...
function M.reset_augroup()
  M.group = vim.api.nvim_create_augroup('neviraide-ui.hacks', { clear = true })
end
...
function M.fix_nohlsearch()
  M.fix_nohlsearch = Util.interval(30, function()
    if vim.o.hlsearch and vim.v.hlsearch == 0 then
      local m = require('neviraide-ui.ui.msg').get('msg_show', 'search_count')
      require('neviraide-ui.message.manager').remove(m)
    end
  end, {
...
function M.draw_cursor()
  require('neviraide-ui.utils.ffi').setcursor_mayforce(true)
end
...
function M.fix_input()
  local function wrap(fn, skip)
    return function(...)
      if skip and skip(...) then return fn(...) end
  ...
  local function skip(expr) return expr ~= nil end
...
function M.cmdline_force_redraw()
  if not require('neviraide-ui.utils.ffi').cmdpreview then return end
