...
local Lsp = require('neviraide-ui.lsp')
local Treesitter = require('neviraide-ui.text.treesitter')
...
M.log = {
  ---@class NeviraideUIHealthLog
  checkhealth = {
    start = function(msg) start(msg or 'neviraide-ui.nvim') end,
    info = function(msg, ...) info(msg:format(...)) end,
    ok = function(msg, ...) ok(msg:format(...)) end,
    warn = function(msg, ...) warn(msg:format(...)) end,
    error = function(msg, ...) error(msg:format(...)) end,
  },
  ---@type NeviraideUIHealthLog
  notify = {
    start = function(msg) end,
    info = function(msg, ...) Util.info(msg:format(...)) end,
    ok = function(msg, ...) end,
    warn = function(msg, ...) Util.warn_once(msg:format(...)) end,
    error = function(msg, ...) Util.error_once(msg:format(...)) end,
  },
}
...
function M.check(opts)
  ...
  if vim.fn.has('nvim-0.8.0') ~= 1 then
    log.error('NeviraideUI needs Neovim >= 0.8.0')
    -- require("neviraide-ui.util").error("NeviraideUI needs Neovim >= 0.9.0 (nightly)")
    if not opts.checkhealth then return end
  else
    log.ok('**Neovim** >= 0.8.0')
    if opts.checkhealth and vim.fn.has('nvim-0.9.0') ~= 1 then
      log.warn(
        '**Neovim** 0.9.0 (nightly) is recommended, since it fixes some issues related to `vim.ui_attach`'
      )
    end
  end
  ...
  for _, ui in ipairs(uis) do
    ...
    for _, ext in ipairs({ 'ext_cmdline', 'ext_popupmenu', 'ext_messages' }) do
      if ui[ext] then
        ok = false
        log.error(
          "You're using a GUI that uses "
            .. ext
            .. ". NeviraideUI can't work when the GUI has "
            .. ext
            .. ' enabled.'
        )
      end
  ...
  if opts.checkhealth then
    if not Util.module_exists('notify') then
      log.warn(
        'NeviraideUI needs nvim-notify for routes using the `notify` view'
      )
      if not opts.checkhealth then return end
    else
    ...
    if vim.o.shortmess:find('S') then
      log.warn(
        'You added `S` to `vim.opt.shortmess`. Search count messages will not be handled by NeviraideUI. So no virtual text for search count.'
      )
    end
...
Either disable the other plugin or set `config.%s.enabled = false` in your **NeviraideUI** config.
  - plugin: %s
  - file: %s
  - line: %s]]):format(
                check.handler_str,
            ...
            log.ok(('`%s` is set to **NeviraideUI**'):format(check.handler_str))
          end
        elseif opts.checkhealth then
          log.warn(
            '`'
              .. check.handler_str
              .. '` is not configured to be handled by **NeviraideUI**'
          )
        end
      end
    end
...
function M.get_source(fn)
  ...
  if source:find('neviraide-ui') then
    ret.plugin = 'neviraide-ui.nvim'
...
M.checker = Util.interval(1000, function()
  if Config.is_running() then M.check({ checkhealth = false }) end
end, {
  enabled = function() return Config.is_running() end,
})
